...
[<SolidComponent>]
let AlertDialogTrigger (props: AlertDialogTrigger) = Kobalte.AlertDialog.Trigger().spread(props)
[<SolidComponent>]
let AlertDialogPortal (props: AlertDialogPortal) = Kobalte.AlertDialog.Portal().spread(props)

[<SolidComponent>]
let AlertDialogOverlay (props: AlertDialogOverlay) =
    let local,others = Solid.splitProps(props,[|"class"|])
    Kobalte.AlertDialog.Overlay(class'=Lib.cn [|
    ...
    Kobalte.AlertDialog.Portal() {
        Types.AlertDialogOverlay() {
            Kobalte.AlertDialog.Content(
                ...
                Kobalte.AlertDialog.CloseButton(class'="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[expanded]:bg-accent data-[expanded]:text-muted-foreground") {
...
let AlertDialogDescription (props: AlertDialogDescription) =
    let local,others = Solid.splitProps(props, [|"class"|])
    Kobalte.AlertDialog.Description(class'=Lib.cn [|"text-sm text-muted-foreground"; local?``class``|]).spread(others)